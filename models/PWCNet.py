...
try:
	...
	from util import correlation # you should consider upgrading python
# end
...
def estimate(moduleNetwork, tensorFirst, tensorSecond):
	...
	if intPreprocessedWidth==intWidth and intPreprocessedHeight == intHeight:
		tensorFlow = moduleNetwork(tensorFirst, tensorSecond)
		tensorFlow = 20.0 * torch.nn.functional.interpolate(input=tensorFlow, size=(intHeight, intWidth), mode='bilinear', align_corners=False)
	else:
		...
		tensorFlow = moduleNetwork(tensorPreprocessedFirst, tensorPreprocessedSecond)
		tensorFlow = 20.0 * torch.nn.functional.interpolate(input=tensorFlow, size=(intHeight, intWidth), mode='bilinear', align_corners=False)

		tensorFlow[:, 0, :, :] *= float(intWidth) / float(intPreprocessedWidth)
		tensorFlow[:, 1, :, :] *= float(intHeight) / float(intPreprocessedHeight)
	return tensorFlow.permute(0, 2, 3, 1)
# end
