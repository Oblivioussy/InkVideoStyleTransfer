...
class CycleGANModel(BaseModel):
    ...
    def clac_GA_common_loss(self, fake_B, lambda_sup, SA=None, edge=True, lambda_newloss=1):
        # calc common loss for video and image
        ...
        loss_G_ink = self.criterionGAN(
            pred_fake_ink, True) * self.opt.lambda_ink * lambda_newloss
        ...
        if SA is not None and torch.sum(SA) > 0.1:
            ...
            # edge loss
            if edge and abs(lambda_sup * lambda_newloss) > 0:
            ...
            if abs(self.opt.lambda_back * lambda_newloss) > 1e-5:
                ...
                loss_back = torch.sum(tmp * bmask) / \
                    torch.sum(bmask) * (0 + 1 / (torch.sum(tmp * emask) / \
                    torch.sum(emask)))
        ...
        if abs(self.opt.lambda_TV * lambda_newloss) > 0:
            # TV loss
    ...
    def getF(self, indexA=0, indexB=None, rev=False):
        # extract feature from frame indexA to frame indexB 
        if indexB is None:
